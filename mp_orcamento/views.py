...
def orcamentos_cliente(request, identificador):         
    cliente = Cliente.objects.get(id=identificador)
    orcamentos = Orcamento.objects.filter(cliente=cliente)    
    return render(request, 'mp_orcamento/cliente.html',
     {'cliente': cliente, 
     'orcamento':orcamentos})

def estatistica_clientes(request):
    maior_custo = 0
    menor_custo = 999999999999
    orcamento_maior_custo = None
    orcamento_menor_custo = None
